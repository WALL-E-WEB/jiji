...
> JavaScript
 ...
 How would you like to define a style for your project?
  ...
  Inspect your JavaScript file(s) // 加载你的js文件
...
--ext 匹配后面类型文件
eslint --ext .js,.html src // 检测src的js和html文件

修复：
eslint index.js --fix

eslint  --fix src/* --ext .js,.vue // 修复src下所有js\vue文件
```
...
```javascript
extends: [
    'plugin:vue/strongly-recommended', //插件中的 extends
...
定义的变量时，避免no-undef 规则将发出警告

```js
 "globals": {
        "customvar1": "writable", // 允许重写变量
        "customvar2": "readonly" // 只读
     	"Promise": "off" // 关闭全局变量
    }
由于历史原因:
...
```
1.下载vscode的ESLint插件；
2.设置vscode快捷键：键盘快捷键》搜索 eslint.executeAutofix 设置快捷键
```
